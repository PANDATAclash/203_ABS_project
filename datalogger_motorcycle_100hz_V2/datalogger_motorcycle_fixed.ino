...
void loop() {
  // ---- BLE commands: '1'=start  '0'=stop  's'=status  'c'=pitch stream ----
  while (BLE.available()) {
    char cmd = BLE.read();
  ...
  if (calibStreaming && rawAccValid) {
    ...
    if (lastCalibStreamMs == 0 ||
        (nowMs - lastCalibStreamMs) >= CALIB_STREAM_INTERVAL_MS) {
    ...
    if (calibrationDone && rawAccValid && gpsValid) {
      ...
      if (now - lastLogMs >= LOG_INTERVAL_MS) {
        if (lastLogMs == 0) {
          ...
          logFile.print(gyrX, 4);     logFile.print(',');
          logFile.print(gyrY, 4);     logFile.print(',');
